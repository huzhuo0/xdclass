...
router.post('/add', (req, res, next) => {
  ...
  let {username} = req.user
    querySql('select id from user where username = ?',[username]).then(searchData=>{
      let user_id = searchData[0].id
      querySql('insert into article(title,content,user_id,create_time) values(?,?,?,NOW())',[title,content,user_id]).then(resd=>{
        res.send({code:0,msg:'新增成功',data:null})                      /* NOW()表示插入当前时间 */
      })
    })
});
